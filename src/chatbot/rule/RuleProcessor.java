...
public class RuleProcessor {
			...
			if (!list.item(i).getNodeName().equals("#text") && !list.item(i).getNodeName().equals("#comment")) {
				...
				if (i + 2 < list.getLength()) {
					...
					if (conditionOperators.peek().equals("not") && !condition.startsWith("!")) {
						condition = "!" + condition + readSubConditions(list.item(i), conditionOperators) + " || ";
					}
					...
					// For operators other than <not>
						...
						condition += readSubConditions(list.item(i), conditionOperators) + conditionOperators.peek();
					}
				}
				
				// For the last <if>
				else {
					condition += readSubConditions(list.item(i), conditionOperators);
				}
			}
		}
	...
	private static String readSubConditions(Node node, Stack<String> conditionOperators) {
		String subCondition = "";
				...
				if (!node.getChildNodes().item(i).getNodeName().equals("#text") && !node.getChildNodes().item(i).getNodeName().equals("#comment")) {
					...
					if (i + 2 < node.getChildNodes().getLength()) {
						...
						if (conditionOperators.peek().equals("not") && !subCondition.startsWith("!")) {
							subCondition = "!" + subCondition + readSubConditions(node.getChildNodes().item(i), conditionOperators) + " || ";
						}
						...
						// For operators other than <not>
							...
							subCondition += readSubConditions(node.getChildNodes().item(i), conditionOperators) + conditionOperators.peek();
						}
					}
					
					// For the last <if>
					else {
						subCondition += readSubConditions(node.getChildNodes().item(i), conditionOperators) + ")";
	...
	private static String normalizeCondition(String variable, String operator, String content) {
		...
		if (content.contains(" * ")) {
			...
			normalCondition = variable + ".lastIndexOf(" + content + ") == " + variable + ".length - " + content + ".length - 1";
		}
	...
	public static ArrayList<Rule> findMatchingRules(String userUtterance) {
		...
		try {
			// Get the rules with satisfying rule condition
			for (Rule rule : rules) {
				if ((boolean) scriptEngine.eval(getConditionVariable(rule, userUtterance))) {
					...
					if (rule.hasVariableInCondition()) {
						...
						int numOfVariables = StringUtils.countMatches(condition, "{");
						
						// Loop through the variables one by one and save them
						for (int i = 0; i < numOfVariables; i++) {
							Matcher matcher = Pattern.compile("\\{(.*?)\\}").matcher(condition);
							while (matcher.find()) {
								// Sub-sting the condition to extract the actual values between { and } 
								String firstPart = condition.substring(0, condition.indexOf("{"));
								firstPart = firstPart.substring(firstPart.lastIndexOf("\"") + 1);
								String secondPart = condition.substring(condition.indexOf("}") + 1);
								secondPart = secondPart.substring(0, secondPart.indexOf("\""));
								
								if (userUtterance.contains(firstPart) && userUtterance.contains(secondPart)) {
									String variableValue = userUtterance.replace(firstPart, "").replace(secondPart, "");
									updateSystemVariable(matcher.group(1), variableValue);
								}
								
								condition = condition.replaceFirst("\\{" + matcher.group(1) + "\\}", ".*");
							}
						}
					}
